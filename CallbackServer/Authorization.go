...
func decodeJwtDialer(cbs CallbackServerSelfHost, funcScope, action string) (company, tenant int, iss string, veeryMessage Result) {
	tokenVals := strings.Split(cbs.Context.Request().Header.Get("authorization"), " ")
	internalAccessToken := cbs.Context.Request().Header.Get("companyinfo")
	if len(tokenVals) > 1 {
